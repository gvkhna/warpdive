...
export default function AppLoader({fileOverride, pid, onFileLoad, onError}: AppLoaderProps) {
  useEffect(() => {
    const loadFile = async () => {
      //
      // const filePath = fileOverride || `/api/builds/${pid}`
      ...
      try {
        if (fileOverride) {
          ...
          const resp = await api.builds[':pid'].$get({param: {pid: pid}})
          if (resp.ok) {
      ...
      <div className='align-items-center flex min-h-screen items-end justify-center px-4 pb-20 pt-4 text-center sm:block sm:p-0'>
        ...
        >
          <div className='mx-auto flex h-12 w-12 items-center justify-center rounded-full'>
            <svg
              className='h-15 w-15 animate-spin text-black'
